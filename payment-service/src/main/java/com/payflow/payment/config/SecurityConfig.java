...
public class SecurityConfig {
    ...
    public OpenAPI customOpenAPI() {
        return new OpenAPI()
                .info(new Info().title("PayFlow Payment & Saga Service").version("1.0.0").description("Distributed payment orchestration, P2P transfers, refunds, and Saga workflow engine"))
                ...
                .exceptionHandling(ex -> ex
                        .authenticationEntryPoint((request, response, authException) -> {
                            ...
                            ErrorResponse error = ErrorResponse.of(
                                    "UNAUTHORIZED",
                                    "Authentication is required to initiate payments",
                                    401,
                        ...
                        .accessDeniedHandler((request, response, accessDeniedException) -> {
                            ...
                            ErrorResponse error = ErrorResponse.of(
                                    "FORBIDDEN",
                                    "Access denied: Insufficient privileges for payment operation",
                                    403,
                                    request.getRequestURI()
                            );
                            response.getOutputStream().write(objectMapper.writeValueAsBytes(ApiResponse.error(error, null)));
