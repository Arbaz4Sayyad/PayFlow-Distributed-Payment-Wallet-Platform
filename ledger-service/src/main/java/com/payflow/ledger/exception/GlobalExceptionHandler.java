...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handlePayFlowException(PayFlowException ex, HttpServletRequest request) {
        ...
        log.warn("Ledger Exception [{}]: {} (Path: {})", ex.getErrorCode(), ex.getMessage(), request.getRequestURI());
    ...
    public ResponseEntity<ApiResponse<Void>> handleAccessDenied(AccessDeniedException ex, HttpServletRequest request) {
        ...
        ErrorResponse errorResponse = ErrorResponse.of(
                "FORBIDDEN",
                "Access denied: You do not have permission to access this ledger data",
                403,
                request.getRequestURI()
        );
    ...
    public ResponseEntity<ApiResponse<Void>> handleGenericException(Exception ex, HttpServletRequest request) {
        String traceId = getTraceId();
        log.error("Unhandled ledger service exception on path {}: {}", request.getRequestURI(), ex.getMessage(), ex);

        ErrorResponse errorResponse = ErrorResponse.of(
                "INTERNAL_SERVER_ERROR",
                "An unexpected error occurred in ledger service. Trace ID: " + traceId,
                500,
                request.getRequestURI()
        );
